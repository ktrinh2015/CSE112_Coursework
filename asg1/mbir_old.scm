...
(for-each (lambda (symfun) 
          (hash-set! *function-table* (car symfun) (cadr symfun)))
   `(
        (+    ,+)
        (-    ,-)
        (*    ,*)
        ...
        ;;(asub , (lambda (x y)  
        ;;(vector-ref (hash-ref *array-table* x) y )))  
        ;;GOD I HATE THIS
...
;;(if (string=? (car expr) "asub") 
;;(don't do things) (map expr to args))\
...
;;check if array ref is in table, evaluate expr at array location
...
;;control transfers to the statement, as for the goto statement.
(define (interp-if args continuation)
    ;;Checking if RELOP IS THERE
    (define temp (car args))
    ;;if < > <= >= !=  exists do eval-expr    
    (if ((hash-ref *function-table* (car temp)) (eval-expr (cadr temp)) (eval-expr (caddr temp)))
...
(define (interp-input args continuation)
    ...
    ;;(when (null? args)
        ;;(begin 
        ;;(display "Hello NULL")
        ;;(interp-program continuation) ;; if args is null Move on
    (if (null? args)
        ;;(begin 
        ;;(display "Hello NULL")
        (interp-program continuation) ;; if args is null Move on
        (let ((number (readnumber args continuation))) ;;if not null
    ...
    ;;(let ((num (readnumber args continuation))) ;;if args not null
    ;;    (if (eof-object? num)
            ;;(printf "*EOF* ~a~n" num)
    ;;        (hash-set! *var-table* 'eof 1)
    ;;        (begin 
    ;;            ;;(display (car args))(newline)
    ;;            (hash-set! *var-table* (car args) num)
                ;;(printf "Var-table is ~a~n" *var-table*)
    ;;            (interp-input args continuation))
    ;;            ))
    ;;(interp-program continuation))
...
(define (scan-for-labels program)
    (define (get-label line)
        (and (not (null? line))
             ...
             (cadr line)))
    (when (not (null? program))
          (let ((label (get-label (car program))));;get-label(car prog)
               (when (symbol? label)
